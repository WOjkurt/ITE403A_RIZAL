//#region node_modules/@angular/platform-server/third_party/domino/bundled-domino.mjs
...
function requireLinkedList() {
	...
	var LinkedList$1 = LinkedList.exports = {
		...
		replace: function(a, b) {
			...
			if (b !== null) LinkedList$1.insertBefore(b, a);
...
function requireNodeUtils() {
	...
	function fallbackRawContentTags(node) {
		...
		while (node) if (node.nodeType === 1) {
			if (node.namespaceURI === NAMESPACE.HTML && hasRawContentFallback[node.tagName]) tags.push(node.localName);
			node = node.parentNode;
		} else if (node.nodeType === 11 && node._host) node = node._host;
	...
	*/
	function escapeMatchingClosingTag(rawText, parentTag) {
		...
		return rawText.replace(new RegExp(escapeRegExp(parentClosingTag), "ig"), (m) => "&lt;" + m.slice(1));
	}
	...
	function escapeMatchingClosingTags(rawText, parentTag, ancestorTags) {
		...
		if (ancestorTags) for (const ancestorTag of ancestorTags) result = escapeMatchingClosingTag(result, ancestorTag);
		return result;
	}
	...
	function findCommentEnd(rawText, index) {
		if (rawText.charAt(index) === ">") return index + 1;
		if (rawText.charAt(index) === "-" && rawText.charAt(index + 1) === ">") return index + 2;
	...
	function serializeOne(kid, parent) {
		...
		switch (kid.nodeType) {
			case 1:
				...
				if (!(html && emptyElements[tagname])) {
					...
					if (hasRawContent[upperTag] && !hasRawContentFallback[upperTag] && ss.includes("</")) {
						...
						for (const fallbackTag of fallbackTags) ss = escapeMatchingClosingTag(ss, fallbackTag);
					}
			...
			case 4:
				...
				if (hasRawContent[parenttag]) s += hasRawContentFallback[parenttag] ? escapeFallbackRawText(kid.data, parent.localName, fallbackRawContentTags(parent.parentNode)) : kid.data;
			...
			case 8:
				...
				if (commentData.includes("</")) {
					const fallbackTags = fallbackRawContentTags(parent);
					for (const fallbackTag of fallbackTags) commentData = escapeMatchingClosingTag(commentData, fallbackTag);
				}
				s += "<!--" + commentData + "-->";
				break;
			case 7:
				...
				if (content.includes("</")) {
					const fallbackTags = fallbackRawContentTags(parent);
					for (const fallbackTag of fallbackTags) content = escapeMatchingClosingTag(content, fallbackTag);
				}
...
function requireElement() {
	...
	Element.prototype = Object.create(ContainerNode.prototype, {
		...
		setAttributeNode: { value: function setAttributeNode(attr) {
			if (attr.ownerElement !== null && attr.ownerElement !== this) utils.InUseAttributeError();
			var result = null;
			...
			if (oldAttrs) {
				...
				else if (attr.ownerElement !== null) utils.InUseAttributeError();
		...
		setAttributeNodeNS: { value: function setAttributeNodeNS(attr) {
			if (attr.ownerElement !== null) utils.InUseAttributeError();
...
function requireNavigatorID() {
	...
	NavigatorID_1 = Object.create(null, {
...
var index = /*@__PURE__*/ getDefaultExportFromCjs(requireLib());
//#endregion
export { index as t };
